...
namespace Sitecore.Foundation.Fields.Fields
{
    public class RolesMultilist : MultilistEx
    {
        ...
        protected override void DoRender(HtmlTextWriter output)
        {
            ...
            {
                output.Write(string.Format("<option value=\"{0}\">{0}</option>", unselectedRole.Name));
            }
            ...
            {
                output.Write(string.Format("<option value=\"{0}\">{0}</option>", selectedRole.Name));
            }
        ...
        {
            return Fields.RolesField.CreateNewRolesField(Source, Value);
        }
    }
}